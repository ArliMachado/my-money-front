...
const BILLING_CYCLE_FORM_ID = 'billingCycleForm';
const CREDIT_LIST = 'credits';
class CreditList extends Component {


    add(index, item = {}) {
        if (!this.props.readOnly) {
            this.props.arrayInsert(BILLING_CYCLE_FORM_ID, CREDIT_LIST, index, item);
        }
    }

    remove(index) {
        if (!this.props.readOnly && this.props.list.length > 1) {
            this.props.arrayRemove(BILLING_CYCLE_FORM_ID, CREDIT_LIST, index);
        }
    }
